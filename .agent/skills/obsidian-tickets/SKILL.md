...
├── _Dashboards/       (7 dashboards: Inbox, Backlog, Active, Next Release, Done, All Tickets, Epics)
├── Tickets/
...
| 9a | Session start, for each ticket with `status: Staging` (runs after Rule #8, so tickets it just moved there are included) | Look for a successful `FamilyHQ-Deploy-Staging` run that started **after** the ticket's `merged` date (`jk run ls FamilyHQ-Deploy-Staging --limit 5`). Found one → **advance it without asking**: set `status: Ready for release`, `staged: <date the run passed>`, bump `updated`, and record the run number in the ticket body. The green run *is* the approval (user's ruling, FHQ-204): the run is started by the `dev` build of a specific merge commit, so "this run covers that merge" is a fact, not a judgement — and a prompt nobody answers leaves the ticket misrepresenting reality (FHQ-196 and FHQ-189 sat in `Staging` for hours after #253/#254 were green). **Prompt only when it is genuinely ambiguous:** no staging run since the merge; a *failed* run between the merge and the green one; or a `master`-cut hotfix — no `dev` run will ever cover it, so look instead for the staging run the `master` build started with a `SEMVER_TAG`. That run also promotes onward, so such a ticket usually clears 9a and 9b in one session — and it still owes its back-merge PR before it is genuinely finished. |
