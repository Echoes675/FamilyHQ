...
- **Local parallelism is capped (FHQ-150):** high xUnit parallelism crashes Playwright's transport (`Stack overflow` in `WrapApiCallAsync`) on local Windows/.NET 10 — CI on the Linux Playwright image is unaffected. The `e2e` verb therefore caps `maxParallelThreads` to **1** for the run (restoring the committed value, 6, afterwards). Override with `-MaxParallelThreads <n>` if you want to try higher locally, but 6 is known to crash on Windows. Note local E2E has a separate open blocker (OAuth login fails locally — FHQ-150), so a local run may not go green even without the crash.
