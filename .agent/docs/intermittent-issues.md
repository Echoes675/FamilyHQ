...
### 12. Weekly-recurring "uniform time" assertions are armed to fail across a DST transition (surfaced by issue 11's zone pin)

**Status:** **latent, not yet observed.** Introduced-as-reachable (not created) by pinning the E2E browser to `Europe/London` — see issue 11. Recorded up front so a first occurrence is recognised instead of investigated from scratch.
**Component:** Simulator recurrence fidelity — `src/FamilyHQ.Core/Calendar/Recurrence/RecurrenceRuleBuilder.Expand` as called by the Simulator's `EventsController.ExpandSeriesInstances`. Assertion side: `DashboardPage.AssertWeeklySeriesOccupiesExactlyItsSlotsAsync`.
...
**Mechanism:** the Simulator expands a weekly series at **fixed UTC instants** — every occurrence is the seed instant plus a whole number of weeks. Real Google anchors a recurrence to `start.timeZone` and holds the *wall clock* across a DST transition. While the browser was UTC the two were indistinguishable, because a UTC browser renders a fixed instant at a fixed wall clock all year. A `Europe/London` browser does not: if the ~15-day span of a 3-occurrence series straddles the late-March or late-October transition, the tiles read e.g. `7:00 PM` then `6:00 PM`, and the uniform-time assertion fails with *"Occurrence N of '…' shows … but the first occurrence shows …"*.

**Window:** roughly two ~2-week periods a year, around the UK DST transitions. Outside those, unreachable.

**Deliberately not fixed here:** the honest fix is Simulator fidelity — expand recurrences anchored to the series' IANA zone rather than to a fixed UTC instant. That is a change to shared recurrence-expansion code used by production paths, which is not something to fold into a timezone-alignment commit. Papering over it by pinning the browser back to UTC would re-open issue 11, and loosening the assertion would delete the only check that the series lands at a consistent time.

**If it occurs:** confirm the run date sits within ~2 weeks of a UK DST transition and that the reported tile times differ by exactly one hour. That combination is this entry — fix the Simulator's expansion to be zone-anchored (matching `project_simulator_google_write_semantics`: the Simulator does not model Google's semantics for free). Any other shape is a different bug.
