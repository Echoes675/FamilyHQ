...
- **Zone-less fallback is deliberate and pinned.** A caller that supplies no zone gets exactly the old fixed-UTC enumeration. That is *exact* for date-anchored all-day masters (which carry no `start.timeZone` and cannot be moved by DST — the dominant real null case), and a missing or unrecognised zone must not fail a legitimate user edit. Production logs a Warning naming the series when it lands there, since that path is not DST-aware.
...
**If the symptom returns:** confirm the run date sits within ~2 weeks of a UK DST transition and that the reported tile times differ by exactly one hour. If so, first check the seeded master actually carries a `StartTimeZone` — a new recurring seed that omits it silently reinstates fixed-UTC expansion, which is the one way this class comes back. Any other shape is a different bug.
