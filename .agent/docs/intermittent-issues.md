...
**Status:** root cause **confirmed and closed by construction** on branch `chore/FHQ-158-pure-unit-tests` — the tests no longer settle on time or scheduling at all, so the window this entry describes cannot be re-entered. Per this file's own convention (see issue 3-legacy): **do not mark Resolved off a handful of green runs** — the original failure was load-dependent, so the bar remains a sustained streak of loaded shared-host CI runs with zero recurrences. The two earlier "fixes" (a wider wall-clock deadline, then a documented-but-unclosed residual) are exactly why the bar is a streak and not an argument.
**Component:** unit-test infra only — `tests/FamilyHQ.WebUi.Tests/Services/SignalRConnectionCoordinatorTests.cs`. No production code involved; the coordinator under test behaved correctly in every occurrence.
...
- A purity guard (`tests/FamilyHQ.Core.Tests/UnitTestPurityGuardTests.cs`) now fails the build if `Task.Delay`/`Thread.Sleep`/`Stopwatch`/`UseInMemoryDatabase` reappear anywhere in `tests/`. This is not belt-and-braces: `Task.Delay` settles had **already** crept back into `TransientHttpRetryHandlerTests` (FHQ-114, batch 5) one batch after the FHQ-125 fix landed, with the same lost-advance shape.

**Local verification:** 20/20 consecutive green runs of the class, ~215 ms per run (previously the waits alone could spend seconds). The load-dependence bar above still applies — a 20-run local streak on an idle machine is not the evidence this entry asks for.
