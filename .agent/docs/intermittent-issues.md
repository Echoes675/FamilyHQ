...
**Component:** E2E date plumbing — `tests-e2e/FamilyHQ.E2E.Common/Hooks/PlaywrightDriver.cs` (browser context zone), `tests-e2e/FamilyHQ.E2E.Common/Helpers/DateExpressionResolver.cs`, `tests-e2e/FamilyHQ.E2E.Steps/WeatherSteps.cs`. Related production-side ticket: FHQ-134 (`DayThemeService` date key), fixed in the same branch but a **separate** defect — the two share a root-cause class, not a code path.
...
**Root cause (confirmed):** the suite had **three** different answers to "what day is it?", and the hourly chain crossed two of them.
- `PlaywrightDriver` created the browser context with **no `TimezoneId`**, so the browser inherited the CI host's zone — **UTC**. Blazor WASM derives `Index.razor`'s `_selectedDate` and renders every `DateTimeOffset` in the *browser's* zone, so the day view asked the API for the **UTC** date (`DayView.razor` → `LoadHourlyAsync` → `api/weather/hourly?date=`).
- The seed resolved "today" through `DateExpressionResolver.ResolveLondon` — **Europe/London**, which at 23:xx UTC during BST is already the **next** day.
- Everything server-side is anchored to Europe/London regardless: the fixture saves a location in Edinburgh (`55.95, -3.19` + a small per-scenario offset), and both the weather ingest (`OpenMeteoWeatherProvider`) and the read window (`WeatherDataPointRepository.GetHourlyAsync`) derive the zone from those coordinates via `GeoTimeZoneLookup`.

Every link in the hourly chain was London **except** the `?date=` query parameter, which was browser-local. For one hour a night the rows were seeded under a date the day view never queried → empty forecast → no `.day-hour-temp`. Note the seed could **not** simply be flipped to UTC: the daily/agenda path takes its window server-side from London (`GetDailyAsync`) and would then have broken in the same window. One zone source had to win, and it had to be London.

**Fix:** the browser is pinned to the zone the test's own fixture implies, and every test-side date calculation is routed through the same source.
- New `tests-e2e/FamilyHQ.E2E.Common/Helpers/BrowserClock.cs` — one place that owns `TimeZoneId = "Europe/London"`, `Today`, and `ToUtcInstant`. `Europe/London` rather than `Europe/Dublin` (identical offsets) because it is what the server actually computes from the fixture's coordinates, so the alignment is a derivation rather than a coincidence. It is also the production-accurate setup: a real kiosk browser sits in the family's zone; a UTC browser paired with a Scottish location was the anomaly.
- `PlaywrightDriver` (and `WarmupHooks`' throwaway context) set `TimezoneId = BrowserClock.TimeZoneId`.
- `DateExpressionResolver.ResolveLondon` is **deleted**, not merely redirected — `Resolve` now answers with the browser-zone date and is the only entry point, so the divergence cannot be reintroduced by picking the wrong overload.
- Every bare `DateTime.Today` in `tests-e2e` (30 sites across 9 files — seeds, day-picker navigation, and month-label assertions) now reads `BrowserClock.Today`. Without this the pin would have *moved* the flake rather than fixed it: those sites compare a **host**-computed date against **browser**-rendered DOM, which is the same divergence in ~30 more places.
- Timed event seeds go on the wire as explicit UTC instants (`BrowserClock.ToUtcInstant`). They previously went naive, and the Simulator's EF converter stamped an unspecified `DateTime` with its own container's zone (UTC), so `"14:30"` silently became `14:30Z`. That rendered correctly only because the browser happened to share the server's zone — true for a UTC CI host and true for a UK developer running both locally, but a coincidence, and pinning the browser independently breaks it. This is what would otherwise have broken `MonthAgendaView.feature` *"Timed events display in 24hr HH:mm format"* (`"14:30 Standup"` → `"15:30 Standup"` during BST) — the only hardcoded wall-clock expectation in the suite.

**Ripple that was assessed and found safe (do not re-walk):** `EventModalTimePicker` (pure browser-internal pixel↔time round-trip, no seed); day-view event *height* assertions (duration, not position); the now-line (visibility only); `RecurringEventTimezone.feature` (asserts an IANA **string** via the Simulator backdoor, the browser's zone never participates); `DayRollover.feature` (`KioskTimeProvider` shifts whole days and every assertion compares against a label captured from the browser earlier in the same scenario); all-day seeds (serialised `date`-only, so no offset can move them). See issue 12 for the one exposure this change *arms*.

**What would confirm it:** a green `Weather.feature` gate run whose E2E phase falls **inside** 23:00–00:00 UTC during BST is the direct evidence. Failing that, the structural argument is the bar: there is now exactly one zone source for the browser, the seed and every test-side date, so the window in which they can disagree no longer exists. A recurrence *outside* that window was never this issue and must be investigated fresh.

**If the symptom returns:** first check whether a new step reintroduced a second clock — a bare `DateTime.Today`/`DateTime.Now` in `tests-e2e`, or a seed that computes a date from anything other than `BrowserClock`/`DateExpressionResolver`. That is the only way this class comes back.

---

### 12. Weekly-recurring "uniform time" assertions are armed to fail across a DST transition (surfaced by issue 11's zone pin) — **and the same defect silently deletes occurrences in production** (FHQ-161)

**Status:** root cause **confirmed by code trace and reproduced as unit tests**; fix implemented on branch `fix/FHQ-161-recurrence-zone-anchor`. **Not yet resolved** — per this file's convention the bar is evidence, and the direct evidence here is a green Deploy-Dev E2E gate whose seeded ~15-day series actually straddles a UK transition. That cannot happen before late October 2026. The structural argument below is the interim bar.
**Component:** **Production**, not merely Simulator fidelity — see the escalation below. `src/FamilyHQ.Core/Calendar/Recurrence/RecurrenceRuleBuilder` (all four enumerators), reached from `CalendarEventService.ReshapeRuleAsync` (production) and the Simulator's `EventsController.ExpandSeriesInstances`. Assertion side: `DashboardPage.AssertWeeklySeriesOccupiesExactlyItsSlotsAsync`.
**Scenarios exposed:** `RecurringEventsDisplay.feature` *"A weekly series syncs into exactly one occurrence per week at a uniform time"* and `RecurringEventsEdit.feature` *"Editing all events from a later occurrence keeps the series on its original dates"*.

**Mechanism:** the engine expanded a series at **fixed UTC instants** — every occurrence is the seed instant plus a whole number of weeks. Real Google anchors a recurrence to `start.timeZone` and holds the *wall clock* across a DST transition. While the browser was UTC the two were indistinguishable, because a UTC browser renders a fixed instant at a fixed wall clock all year. A `Europe/London` browser does not: if the ~15-day span of a 3-occurrence series straddles the late-March or late-October transition, the tiles read e.g. `7:00 PM` then `6:00 PM`, and the uniform-time assertion fails with *"Occurrence N of '…' shows … but the first occurrence shows …"*.

**Window:** roughly two ~2-week periods a year, around the UK DST transitions. Outside those, unreachable *as an E2E symptom* — the production defect below is reachable whenever a user's series crosses a transition, which is not rare.

**Escalation — this was never only a test-double issue (FHQ-161 investigation, 2026-08-17).** The entry originally scoped this to "Simulator recurrence fidelity" because `RecurrenceRuleBuilder.Expand` has exactly one caller, the Simulator (production never expands a series — Google expands server-side and sync stores the returned instances). That reading was wrong, because `Expand` and `CountOccurrencesBefore` are two entry points onto the **same private enumerator**, and `CountOccurrencesBefore` **is** production: `CalendarEventService.ReshapeRuleAsync`, on the "this and following" edit path, for COUNT-bounded series.

Both of that method's inputs are true wall-clock-anchored instants — the master DTSTART fetched from Google, and the split instance's `Start` as synced into the DB — while only the enumeration between them was fixed-UTC. Crossing a **fall-back** transition, the enumerated twin of the split occurrence lands an hour early, is wrongly counted as "before", and the forward series' remaining COUNT comes out one too low. Worked example — weekly Tue 19:00 `Europe/London`, `COUNT=5` from Tue 13 Oct 2026, clocks back Sun 25 Oct:

| # | True (Google) | Enumerated (before the fix) |
|---|---------------|-----------------------------|
| 1 | 13 Oct 18:00Z | 13 Oct 18:00Z |
| 2 | 20 Oct 18:00Z | 20 Oct 18:00Z |
| 3 | 27 Oct **19:00Z** | 27 Oct 18:00Z |
| 4 | 3 Nov **19:00Z** | 3 Nov 18:00Z |
| 5 | 10 Nov **19:00Z** | 10 Nov 18:00Z |

Splitting at occurrence 4 gives `before = 4` instead of 3, so the forward series is created with `COUNT=1` and **occurrence 5 is silently deleted from the family's calendar** — no error, no log. Splitting at the final occurrence of a `COUNT=4` series drives `remaining` to 0 and throws `InvalidSeriesSplitException`, rejecting a legitimate edit with a false explanation. Spring-forward is benign for this path (the twin lands an hour *late* and is correctly excluded), which is why late October is the real deadline. All four frequencies were affected, not just weekly: Daily/Weekly stepped exact 24-hour spans, Monthly/Yearly built UTC midnight plus a UTC time-of-day.

**Fix:** the engine now steps in **wall-clock terms inside the series' zone** and maps each reading to an instant, so a 19:00 event stays 19:00 and its UTC instant moves.
- The zone is **injected**, not resolved in Core. `FamilyHQ.WebUi` (Blazor WASM) project-references `FamilyHQ.Core`, so a tz-database package there would ship the whole TZDB in the payload the Pi kiosk downloads. `FamilyHQ.Core.csproj` is unchanged; `IRecurrenceTimeZone` / `IRecurrenceTimeZoneFactory` carry no dependency, and `NodaTimeRecurrenceTimeZoneFactory` (Services, and the Simulator's own copy) wraps the static tzdb lookup behind an injectable singleton. `TimeZoneInfo` was not an option — the CI/test runtime is globalization-invariant.
- `SeriesMaster` now carries Google's `start.timeZone`, read off the `events.get` payload by `GoogleCalendarClient`, and `ReshapeRuleAsync` counts in that zone.
- The Simulator expands using the master's stored `StartTimeZone`, which is now plumbed through both seeding paths (config template and backdoor). The E2E recurring seeds set it to `BrowserClock.TimeZoneId`, so the Simulator, the browser and the server agree on the zone by construction rather than by coincidence — the same discipline issue 11 applied to dates. **No `.feature` file changed.**
- **The zone argument is REQUIRED, and its fixed-UTC fallback is named.** `Expand`/`CountOccurrencesBefore` take no default, so a new caller cannot silently inherit the buggy behaviour — opting into it means writing `FixedUtcRecurrenceTimeZone.Instance` (public, in Core, no tz-database dependency). The fallback itself is deliberate and pinned: it is *exact* for date-anchored all-day masters (which carry no `start.timeZone` and cannot be moved by DST — the dominant real null case), and a missing or unrecognised zone must not fail a legitimate user edit.
- **The fallback log is levelled by whether it is actually a problem.** `CalendarEventService` warns only for a **timed** zone-less series — the one case that is not DST-aware. An all-day series (exact under fixed UTC) and a degraded local-row anchor (already reported at Warning by the master fetch, and incapable of carrying a zone) log at `Debug`. Warning on the guaranteed-benign paths would have buried the only signal the fallback has.
- Ambiguous wall clocks (the repeated hour at a fall-back) resolve to the **earlier** instant; skipped ones (the spring-forward gap) shift **forward** by the gap. A rule step can therefore never throw or lose an occurrence.

**Evidence:** 38 new unit tests, mutation-checked — reverting the enumerator to fixed-UTC fails 12 of them (all four frequencies, both transition directions, the ambiguous/skipped edges, the pure count seam, and both production failure modes at `ReshapeRuleAsync`); not plumbing the zone from `CalendarEventService` fails exactly the two production-seam tests; dropping `start.timeZone` from the client fails only the client test. Full unit suite 1356 → 1394 passing, zero failures. The spring transition's benign behaviour is pinned in both the pure and service seams so it cannot silently regress, and the Warning-vs-Debug split on the fallback is pinned so the diagnostic cannot be re-buried in noise.

**What would confirm it:** a green Deploy-Dev E2E gate run in which the seeded series genuinely straddles a UK transition (late Oct 2026 / late Mar 2027). Until then the structural argument is the bar: the Simulator, the browser and the server now derive from one zone, and production's split count enumerates in the series' own zone rather than UTC.

**If the symptom returns:** confirm the run date sits within ~2 weeks of a UK DST transition and that the reported tile times differ by exactly one hour. If so, first check the seeded master actually carries a `StartTimeZone` — a new recurring seed that omits it makes the Simulator fall back to fixed-UTC expansion, which is the one way this class comes back. (The engine itself can no longer be got wrong by omission: its zone argument is required.) Any other shape is a different bug.
...
**2026-08-16 (FHQ-134 branch) — third occurrence, Wedge B again, and it exposes a gap in the FHQ-148 instrumentation.** Signature confirmed by the documented recipe: all four services `UP`, **no** testhost/vstest process, **no** browser process, launching `pwsh` at **0.00s CPU delta over 5s**, output file 0 bytes — ~3 hours after boot. The stack had booted at 18:48 (three `dotnet` service processes with normal CPU); the test phase never started.

Two things worth acting on next time:
- **The `dotnet-test` hard timeout did not abort it.** The phase timeouts are supposed to make exactly this survivable (default 30 min), yet the run sat for ~3h. So either the wedge happens *before* the `dotnet-test` phase timer is armed (consistent with Wedge B — it stalls between `boot` and `dotnet-test`), or the timer is not enforced on that boundary. **Next occurrence: check whether any `PHASE start` line for `playwright-install`/`dotnet-test` was ever emitted** — that single fact distinguishes the two, and it is the missing piece for pinning Wedge B.
- **Do not buffer the runner's output.** This run was launched as `... | Out-String` (to filter in memory), which meant the phase log was invisible for the entire 3 hours — the instrumentation was working *and unreadable*. Stream it instead: `pwsh scripts/dev-stack.ps1 e2e ... *>&1 | Tee-Object -FilePath <log>`, then read the log while it runs. The local-stack skill's "capture the full output and filter in memory" advice is for *short* verbs; for `e2e` it defeats the phase logging that exists to diagnose this entry.

Recovery used (as documented, worked cleanly): `Stop-Process` the launcher, then `dev-stack.ps1 down` — which reconciled all three stale services and stopped the Postgres container.
...
### 10. SignalR coordinator unit test times out waiting for the FakeTimeProvider-driven restart continuation under CI load (FHQ-125 → root cause closed in FHQ-158)

**Status:** root cause **confirmed and closed by construction** on branch `chore/FHQ-158-pure-unit-tests` — no *positive* assertion in the affected tests settles on time or scheduling any more; every advance waits on the timer registration it acts on. Two scheduling constructs are **deliberately retained**, so "at all" would be an overclaim: `YieldAsync` (a fixed 20-yield budget) still backs the **negative** assertions ("nothing has fired yet"), where a starved budget can only produce a false pass and never a false CI failure — and no finite wait can prove a negative; and real-clock `WaitAsync` deadlines remain as failure-path tripwires that turn a hang into a legible 5s failure. Per this file's own convention (see issue 3-legacy): **do not mark Resolved off a handful of green runs** — the original failure was load-dependent, so the bar remains a sustained streak of loaded shared-host CI runs with zero recurrences. The two earlier "fixes" (a wider wall-clock deadline, then a documented-but-unclosed residual) are exactly why the bar is a streak and not an argument.
**Component:** unit-test infra only — `tests/FamilyHQ.WebUi.Tests/Services/SignalRConnectionCoordinatorTests.cs`, and (found later, see below) `tests/FamilyHQ.WebUi.Tests/Services/VersionServiceTests.cs`. No production code involved; the coordinator under test behaved correctly in every occurrence.
...
**Occurrences:**
- branch build `fix/FHQ-91-google-client-timeouts` #1 (2026-08-14), `OnStartFailed_SchedulesRestart_AfterInitialDelay`.
- **Local, 2026-08-16**, branch `fix/FHQ-134-160-theme-local-date` (touches no WebUi/SignalR code), `RestartAuthFailure_LogsWarning_AndSchedulesAnotherAttempt` — **with the wall-clock `WaitUntilAsync` fix already in place**. 1 failure / 244 pass; the same project then passed 4/4 consecutively on an idle machine.
- **master build `#59`** (2026-08-15), `RestartDelays_AreCappedAtMaxRetryDelay` — on source **byte-identical to a green `dev`**, which is the strongest available evidence that nothing in the diff was involved. This one had real cost: it broke the release chain, so `031406f` never got a SemVer tag, images, or a Deploy-Staging trigger. It is the occurrence that motivated FHQ-158 and the third time this entry's symptom appeared.

**What actually fixed it (FHQ-158).** The residual documented below was the whole defect, not a footnote: the trailing settle after `WaitUntilAsync` was still yield-based, so a test's next `Advance` could land on a clock with **no timer armed**. `FakeTimeProvider` drops such an advance **silently** — the coordinator's loop then arms its 10 s delay against the already-advanced clock (due at t=35 instead of t=25), attempt 3 never fires, and the wait dies at its 5 s deadline pointing at the wrong thing entirely. The fix removes the ordering assumption rather than widening any window:
- `TimerArmedTimeProvider` decorates `FakeTimeProvider` and counts `CreateTimer` calls, incrementing **after** the inner call returns so the timer is provably registered before the count is observable. `AdvanceOnNextTimerAsync` holds each advance until a timer has been armed that no earlier advance consumed.
- Negative assertions of the shape `Advance(delay - 100ms)` → assert nothing fired → `Advance(100ms)` split **one** armed timer across **two** advances, so their second half is a plain non-waiting advance. Getting this wrong hangs those tests rather than failing them.
- `WaitUntilAsync` (poll + 10 ms real `Task.Delay`, 5 s deadline) is **deleted**. Waits now go through `AwaitableCounter`, signalled from inside the coordinator's own continuation, so there is no settle window and no `Stopwatch`/`Task.Delay` left in the file.
- Assertions that read state written *after* the awaited condition — the failed attempt's `Warning` log, `ConnectionRestored` — now wait on that later observable (the re-armed timer, or the event) instead of relying on a trailing yield to have let it happen.
- Regression test `RestartLoop_WhenTheNextAdvanceIsIssuedMidAttempt_TheAdvanceIsNotLost` parks the restart callback to force the interleaving deterministically. Verified by mutation: swap the held advance for a plain `Advance` and it fails in 5 s.
- A purity guard (`tests/FamilyHQ.Core.Tests/UnitTestPurityGuardTests.cs`) now fails the build if `Task.Delay`/`Thread.Sleep`/`Stopwatch`/`UseInMemoryDatabase` reappear in `tests/` — **with five allow-listed exceptions**, all `UseInMemoryDatabase` in `FamilyHQ.Simulator.Tests/Controllers/` (`Calendars`, `Events`, `OAuth`, `SimulatorConfig`, `Webhook`), deferred to [[FHQ-162]] because every alternative needs a production change in `tools/FamilyHQ.Simulator` or a new package. A second test asserts each allow-list entry still applies, so the list cannot rot into a blanket permit. This is not belt-and-braces: `Task.Delay` settles had **already** crept back into `TransientHttpRetryHandlerTests` (FHQ-114, batch 5) one batch after the FHQ-125 fix landed, with the same lost-advance shape. Read a green guard as "these four constructs are absent", **not** as "this defect class is impossible": it is a lexical scan (comments and string literals are masked, but `using static`/alias evasion is not caught), and `Task.Yield`, `Task.WaitAsync(timeout)` and `SemaphoreSlim.WaitAsync(timeout)` are deliberately unbanned because they are the retained mechanisms above. A fixed yield budget settling a *positive* assertion is still this bug and only review will catch it.
- **The same defect was still live in `VersionServiceTests` when the branch was reviewed** — three tests (`...DiffersInPatch_RaisesUpdateAvailableExactlyOnce`, `...AfterUpdateAvailableFires_FurtherCallsDoNotRefire`, `...InvokesLocationReload_AfterFiveSeconds`) started `CheckAsync()` un-awaited and plain-`Advance`d the fake clock. `VersionService.CheckAsync` awaits the health fetch **before** arming its reload delay, so the advance could land on a clock with no timer. Worse than the original: `await checkTask` had no deadline, so a lost advance **hung forever** instead of failing in 5s — reproduced by injecting a 150 ms async gap before the delay, which ran the suite to a 150 s kill with no failure reported. Now converted to `AdvanceOnNextTimerAsync` (with the 4990/20 split's second half left as a plain advance) and every bare `await checkTask` given a 5s `WaitAsync` tripwire. The same injected gap now passes in ~0.6 s. **Lesson: the guard did not and could not catch this** — the file contained no `Task.Delay`; the defect is the *shape* of the advance, which no regex sees.

**Local verification:** 20/20 consecutive green runs of the class, ~215 ms per run (previously the waits alone could spend seconds). Re-verified after the review fixes **under deliberate CPU contention** (the recipe below, plus six spin loops on an 8-core host): `SignalRConnectionCoordinatorTests` 12/12 green (23 tests, 1–5 s per run) and `VersionServiceTests` 15/15 green (8 tests, 0.3–0.9 s per run). The load-dependence bar above still applies — a local streak, contended or not, is not the evidence this entry asks for.

**First local reproduction recipe (new — the entry previously had none).** The failure appears only under CPU contention, so run the unit-test projects *concurrently* rather than sequentially: launch `FamilyHQ.Core.Tests`, `FamilyHQ.Services.Tests`, `FamilyHQ.WebApi.Tests` and `FamilyHQ.Simulator.Tests` as background jobs and `FamilyHQ.WebUi.Tests` alongside them. That reproduced it on the first attempt; it is not reliable (a second identical attempt passed), but it is far better than the previous "unreproducible on an idle machine".

**Triage of the 2026-08-16 occurrence:** it is a **later-attempt** wait on a *different* test from the original, which this entry's own guidance says to triage against the known residual below — and it fits. `RestartAuthFailure_...` waits on the first attempt (`WaitUntilAsync(() => calls == 1)`), then settles with the yield-based `YieldAsync()` before `time.Advance(MaxDelay - 100ms)`. If the coordinator has not yet registered its *next* fake-timer delay when that `Advance` lands, the advance is missed and the following wait fails at its deadline. That is exactly the documented residual window, not a regression of the helper fix — so per the guidance, **do not inflate the deadline or touch `WaitUntilAsync`**. Closing it means making the trailing settle deterministic (wait for the next timer to be armed, e.g. the `TimerArmedTimeProvider` pattern already used in `DayThemeSchedulerServiceTests`) rather than yield-based.
...
**Fix, first attempt (2026-08-14 — superseded by FHQ-158 above; kept because the reasoning is the trap):** `WaitUntilAsync` was made wall-clock-bounded: it polls the condition with a `Task.Yield()` plus a **10 ms real `Task.Delay`** between checks, up to a generous 5 s deadline. The real delay guarantees the pool wall-clock time to make progress regardless of load; the deadline only bounds the failure path (normal-case cost is unchanged — the condition is typically already true at the first check because FakeTimeProvider continuations usually run inline within `Advance`). This is the same accepted bounded real-delay polling pattern already used in `JwtRenewalServiceTests`. `YieldAsync` (fixed 20-yield budget) was deliberately left as-is: it backs **negative** assertions ("nothing fired yet"), where a starved budget can only produce a false pass in buggy-code scenarios, never a false failure — it cannot flake CI, and no finite wait can prove a negative.

**Known residual (accepted at the time — CLOSED by FHQ-158):** a window remained where a test's `Advance` ran before the coordinator's loop had registered its *next* fake-timer delay (the trailing settle after `WaitUntilAsync` was still yield-based); the timer was then due 10 fake-seconds after creation and never advanced past. It was recorded as "theoretical"; it was in fact the mechanism of both later occurrences, including master `#59`. **Lesson worth keeping: a residual that is only argued to be unlikely is not bounded — this one recurred twice within two days of being written down.**

**If the symptom returns:** first confirm which wait failed — the file no longer contains `WaitUntilAsync`, so a stack frame naming it means the tests were reverted, not that this entry recurred.
- An `AwaitableCounter` timeout ("Expected at least N occurrence(s)…") means the coordinator genuinely did not invoke its callback: check CI host contention (parallel jobs on the shared host, see `project_ci_shared_host`) and treat it as a real hang, not a settle problem.
- A `TimerArmedTimeProvider` timeout ("No timer was armed on the fake clock…") means the coordinator never entered its next delay — look at the loop's catch/log path, not at the test.
- A test that **hangs** rather than fails is the same defect wearing a worse disguise: a task driven by a fake clock was awaited with no deadline, so a lost advance parks forever and the CI job dies on its own timeout with nothing to read. Every `await` on a fake-clock-driven task needs a `WaitAsync` tripwire; grep for a bare `await` of a task started but not awaited on the line it was created.
- Either way: **do not fix by inflating a deadline or a yield count.** A time-free budget was the original defect, a wider deadline was the first failed fix, and both are now the documented wrong answer. The deadlines here bound the failure path only; making them larger buys nothing but a slower red build.
