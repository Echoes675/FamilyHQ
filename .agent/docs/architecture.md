...
- **TransientHttpRetryHandler** (`DelegatingHandler`, FHQ-114): retry for the three non-Google outbound clients — ip-api, Nominatim, Open-Meteo — all registered in `AddFamilyHqServices`. Retries idempotent (GET/HEAD) requests on 408/429/5xx and connection failures, honouring `Retry-After` and ip-api's `X-Ttl`, otherwise exponential backoff with full jitter. A hint longer than `MaxRetryDelay` surfaces the response instead of sleeping. Sleeps happen inside `SendAsync`, so each client's `Timeout` is the TOTAL budget for the attempt+backoff sequence — see `ExternalHttpResilienceOptions` for the worst-case arithmetic. (Contrast `ResilientGoogleCalendarClient`, which decorates an interface because the Google SDK is not a plain `HttpClient`.)
