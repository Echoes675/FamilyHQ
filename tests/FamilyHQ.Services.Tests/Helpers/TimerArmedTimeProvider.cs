...
/// source. Settling on a real interval only narrows the window; observing the registration closes it.
/// </para>
/// <para>
/// Duplicated (rather than shared) with the copy in <c>FamilyHQ.WebUi.Tests</c>: there is no shared
/// test-infra assembly, and the CI unit-test stage runs <c>tests/*/*.csproj</c>, so adding one would
/// change the build shape for ~40 lines. Keep the two byte-identical apart from the namespace and
/// these comments; extracting them into a shared assembly is tracked as FHQ-163.
/// </para>
/// </summary>
